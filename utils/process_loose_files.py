...
from utils.file_utils import get_album_folders, get_mp3_files_in_album, get_mp3_files_in_directory
...
def remove_string_from_filename(file_path, string_to_remove):
    """
    Remove a string from the filename.
    
    Args:
        file_path: Path to the file
        string_to_remove: String to remove from the filename
...
def process_loose_files(source_folder, output_folder, string_to_remove):
    """
    Process loose MP3 files: remove string from filename and move to output folder.
    ...
    Args:
        ...
        string_to_remove: String to remove from filenames
    """
    ...
    for file_path in mp3_files:
        try:
            ...
            new_filename = remove_string_from_filename(file_path, string_to_remove)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python process_loose_files.py <source_folder> <output_folder> [string_to_remove]")
        sys.exit(1)
    
    source_folder = sys.argv[1]
    output_folder = sys.argv[2]
    string_to_remove = sys.argv[3] if len(sys.argv) > 3 else ""
    ...
    # Process the files
    process_loose_files(source_folder, output_folder, string_to_remove)
